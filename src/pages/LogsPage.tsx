      ...
      {activeTab === "actions" && (
        <div className="logs-actions-pane">
          <div className="logs-filter-bar">
            ...
            <input type="search" className="logs-filter-input" placeholder={t.filterEvents}
              value={filterText} onChange={e => setFilterText(e.target.value)} />
            {filterText && (
              <button type="button" className="dsc-sb-search-clear" onClick={() => setFilterText("")} aria-label={t.clear}><i className="ti ti-x" /></button>
            )}
        ...
        <div className="logs-backend-pane">
          ...
          {backendLogLoading ? (
            <div className="dsc-loading-center" style={{ padding: "3rem" }}>
              <div className="spinner" /><span>{t.loadingLog}</span>
            </div>
          ) : backendLogError ? (
